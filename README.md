...
Set `COLLECTION_ADDRESS` to a Victoria street address such as `1500 Fairfield Rd` to show City garbage/organics and CRD recycling as all-day events with icons. The API resolves the address separately with each provider's ReCollect widget service, reads its dated events, and refreshes them every 12 hours. Use the street address only (no postal code); leave it unset to disable the events. Collection events do not need a Google connection and contain no links.
